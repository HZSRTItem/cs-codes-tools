...
namespace SrtArticleCSA
{
    class SrtArt
    {
        ...
        private ArtInfos Articles = new ArtInfos();
        /// <summary>
        /// IntList
        /// </summary>
        private List<int> IntList = new List<int>(StaticInfo.N_ARTS_INIT);
        /// <summary>
        /// 分词器
        /// </summary>
        ...
        /// <returns></returns>
        private bool ContainsFind(List<string> infos)
        {
            bool b = false;
            for (int i = 0; i < Articles.Count; i++)
            {
                ...
                {
                    if (Articles[i].Title.Contains(item))
                    {
                        IntList[Articles[i].Number - 1]++;
                        b = true;
                    }
                }
            }
        ...
        public bool PrintAllArtInfo(int n = -1)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                UUtils.PrintFormat(n: 13, s: string.Format("    {0:D3}      {1}",
                    Articles[n_select].Number, Articles[n_select].Title));
        ...
        public bool PrintFindInfo(int n = -1)
        {
            ...
            Console.WriteLine("Title : {0}", Articles[n].Title);

            Console.WriteLine("Info of Article: ");
            for (int i = 0; i < Articles[n].AInfo.Count; i++)
            {
                Console.WriteLine("    {0, -3} {1, -4} {2}", i + 1
                    , Articles[n].AInfo.GetMark(i), Articles[n].AInfo.GetInfo(i));
            }
            return true;
        }
        ...
        public bool PrintAll()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                UUtils.PrintFormat(n: 13, s: string.Format("    {0:D3}      {1}"
                    , Articles[i].Number, Articles[i].Title));
            }
            Console.WriteLine("    --------------------------------------------------------------------------");
        ...
        public bool AddInfo(int n, string mark, string info)
        {
            ...
            Articles[n - 1].AInfo.Add(mark, info);
            return false;
        }
        ...
        /// <param name="info"></param>
        /// <returns></returns>
        public bool AddOneArticle(string art_name, string[] infos = null)
        {
            ...
            int n_add = Articles.Count + 1;
            string infos_str = "";
            if (infos != null)
            {
                infos_str = string.Join("\n", infos);
            }
            OneArt oneArt = new OneArt { Number = n_add, Title = art_name };
            oneArt.AInfo = new ArtInfo(infos_str);
            ...
            if (y_n == 'y') // 要添加
            {
                ...
                Articles.SaveAll(StaticInfo.InfoFileName);
                Process.Start(dir1);
        ...
        /// 通过RIS文件添加一个Article
        /// </summary>
        /// <param name="ris_file"></param>
        /// <returns></returns>
        public bool AddByRisFile(string ris_file)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                ss.Add(UUtils.MarkInfoToStr(line_mark, line_info));
            }

            return AddOneArticle(title_name, infos: ss.ToArray()); ;
        }
        #endregion
        ...
        /// <returns></returns>
        public bool FindByTitleInfo(string[] infos, int n = -1)
        {
            List<string> fencis = new List<string>(256);
            string info = "    ";
            for (int i = 0; i < infos.Length; i++)
            {
                string[] ss = FenCi(infos[i]);
                fencis.AddRange(ss);
                info += infos[i] + " ";
            }
            info += "\n";
            if (!ContainsFind(fencis))
            {
                Console.WriteLine("Not find Article by follow info:");
                UUtils.PrintFormat(n: 4, s: info);
            }
            Console.WriteLine("Find Article by follow info:");
            UUtils.PrintFormat(n: 4, s: info);
        ...
        /// <param name="n"></param>
        /// <returns></returns>
        public bool DeleteArticle(int n)
        {
        ...
        /// 删除第n个文章的所有的信息
        /// </summary>
        /// <param name="n_art"></param>
        /// <returns></returns>
        public bool DeleteInfoAll(int n_art)
        {
            Console.WriteLine("DeleteInfoAll");
            return true;
        }
        ...
        /// <param name="mark"></param>
        /// <returns></returns>
        public bool DeleteInfoByMark(int n_art, string mark)
        {
            Console.WriteLine("DeleteInfoByMark");
            return true;
        }
        ...
        /// </summary>
        /// <param name="n_art"></param>
        /// <param name="n_mark"></param>
        /// <returns></returns>
        public bool DeleteInfoByN(int n_art, int n_mark)
        {
            if (n_art > Articles.Count | n_art <= 0)
            {
                Console.WriteLine("Not find number article:{0}", n_art);
                return false;
            }
            OneArt oneArt = Articles[n_art - 1];
            if (n_mark > oneArt.AInfo.Count | n_art <= 0)
            {
                Console.WriteLine("Title: {0}\n\n", oneArt.Title);
                Console.WriteLine("Not find number info:{0}", n_art);
                return false;
            }
            n_mark = n_mark - 1;
            Console.WriteLine("Number: {0}", n_art);
            Console.WriteLine("Title: {0}", oneArt.Title);
            Console.WriteLine("Info of Article: ");
            for (int i = 0; i < oneArt.AInfo.Count; i++)
            {
                if (n_mark == i)
                {
                    Console.WriteLine("    * {0, -3} {1, -4} {2}", i + 1
                    , oneArt.AInfo.GetMark(i), oneArt.AInfo.GetInfo(i));
                }
                else
                {
                    Console.WriteLine("      {0, -3} {1, -4} {2}", i + 1
            ...
            Console.WriteLine("    * {0, -3} {1, -4} {2}", n_mark + 1
                    , oneArt.AInfo.GetMark(n_mark), oneArt.AInfo.GetInfo(n_mark));
            Console.Write("Whether to delete this info? [y/n]: ");
            char y_n = (char)Console.Read();
            if (y_n == 'y')
            {
                Articles[n_art - 1].AInfo.Delete(n_mark);
                Console.WriteLine("Already delete!");
            }
            return true;
        }
        ...
        public bool SaveAll(string save_file)
        {
            return Articles.SaveAll(save_file);
        }
    }
    ...
    public class OneArt
    {
        ...
        public bool DcInfo()
        {
            ...
            {
                Info += UUtils.MarkInfoToStr(AInfo.GetMark(i), AInfo.GetInfo(i));
                Info += "\n";
            }
            return true;
        }
    ...
    public class ArtInfo
    {
        ...
        /// 标签
        /// </summary>
        private string[] Marks = new string[StaticInfo.N_INFOS_INIT];
        /// <summary>
        /// 标签信息
        /// </summary>
        private string[] Infos = new string[StaticInfo.N_INFOS_INIT];
        ...
        private void EnDcInfo(string info)
        {
        ...
        /// <param name="info"></param>
        /// <returns></returns>
        public bool Add(string mark, string info)
        {
            bool b = true;
            if (Count >= Marks.Length)
            {
                string[] marks = new string[Marks.Length + StaticInfo.N_INFOS_INIT];
                string[] infos = new string[Marks.Length + StaticInfo.N_INFOS_INIT];
            ...
            Marks[Count] = mark.Trim();
            Infos[Count] = info.Trim();
            Count++;
            return b;
        }
        ...
        /// <param name="i"></param>
        /// <returns></returns>
        public bool Delete(int n_mark)
        {
            if (n_mark >= Count | n_mark < 0)
            {
                return false;
            }
            Count--;
            for (int i = n_mark; i < Count; i++)
            {
                Marks[i] = Marks[i + 1];
                Infos[i] = Infos[i + 1];
            }
    ...
    public class ArtInfos
    {
        ...
        {
            // 需要找到所有的文件名，文件名编号
            Arts = new List<OneArt>(StaticInfo.N_ARTS_INIT);
            ReadAll(StaticInfo.InfoFileName, Arts);
            return true;
        }
        #endregion
        ...
        /// 文章的数量
        /// </summary>
        public int Count { get { return Arts.Count; } }

        /// <summary>
        /// 通过索引获得第 i 个文章的信息
        /// </summary>
        /// <param name="i">第 i 个文章的索引</param>
        /// <returns>一个文章</returns>
        public OneArt this[int i]
        {
            get { return Arts[i]; }
        }
        #endregion
        ...
        /// 添加一个文章
        /// </summary>
        /// <param name="oneArt"></param>
        /// <returns></returns>
        public bool AddArticle(OneArt oneArt)
        {
            Arts.Add(oneArt);
            return true;
        }
        ...
        /// <param name="info_file_name">信息文件 .xml</param>
        /// <returns></returns>
        private bool ReadAll(string info_file_name, List<OneArt> arts)
        {
            if (File.Exists(info_file_name))
            {
                using (FileStream fs = File.OpenRead(info_file_name))
                {
                    OneArt[] arts0 = (OneArt[])XmlSer.Deserialize(fs);
                    for (int i = 0; i < arts0.Length; i++)
                    {
                        arts0[i].AInfo = new ArtInfo(arts0[i].GetInfo());
                        arts.Add(arts0[i]);
                    }
                }
                return false;
