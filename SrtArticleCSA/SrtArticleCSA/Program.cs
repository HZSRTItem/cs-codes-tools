...
namespace SrtArticleCSA
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (args.Length == 0)
            {
                Console.WriteLine(Usage.UsageAll());
                Console.WriteLine("");
                Console.WriteLine(Usage.AddUsage());
                Console.WriteLine("");
                Console.WriteLine(Usage.FindUsage());
                Console.WriteLine("");
        ...
        private static bool Add(string[] args)
        {
            if (args.Length < 2)
            {
                Console.WriteLine("Can not find article name.");
                Console.WriteLine(Usage.AddUsage());
                return false;
            }
            ...
            else
            {
                ...
                OSrtArt.AddOneArticle(art_title, infos: tt);
            }
        ...
        private static bool Delete(string[] args)
        {
            if (args.Length <= 1)
            {
                Console.WriteLine("Please enter \"article\" or \"info\" to delete.");
                Console.WriteLine(Usage.DeleteUsage());
                return false;
            }
        ...
        private static bool BackFile()
        {
            ...
            {
                if (File.Exists(StaticInfo.InfoFileName))
                {
    ...
    class StaticInfo
    {
        static StaticInfo()
        {
            //ArticleDirName = @"D:\code\cs\SrtArticleCSA\SrtArticleCSA\bin\Debug\Test1";
            ArticleDirName = @"D:\GroupMeeting\Articles";
            InfoFileName = Path.Combine(ArticleDirName, "Info.xml");
            BackDirName = Path.Combine(ArticleDirName, "back");
            if (!Directory.Exists(ArticleDirName))
            {
                Directory.CreateDirectory(ArticleDirName);
            }
            if (!Directory.Exists(BackDirName))
            {
                Directory.CreateDirectory(BackDirName);
            }
        }
        ...
        public static string ArticleDirName = null;
        /// <summary>
        /// 信息文件
        /// </summary>
        ...
        /// 备份文件的位置
        /// </summary>
        public static string BackDirName = null;
        /// <summary>
        /// 初始的可添加的全部的文献量
        /// </summary>
    ...
    class Usage
    {
        public static string AddUsage()
        {
            string usage = "add: add a article of info to library\n" +
                "    if   | 'info': [n] [mark] [info]\n" +
                "    else | [title] [opt:-i +mark +info]* [info*]\n" +
                "           [-ris *.ris]: import articles using RIS files";
            return usage;
        }
        ...
        {
            string usage = "srta add/find/delete\n" +
                "    add: add a article of info to library\n" +
                "    find: find article by n or article title or all\n" +
                "    delete: delete article or info by number or mark";
            return usage;
        }
    }
