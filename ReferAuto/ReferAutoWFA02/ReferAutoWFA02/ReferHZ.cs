...
{    /// <summary>
    ...
    class ReferHZ
    {
        ...
        public string SaveToJson(string json_file)
        {
            if (json_file == "")
            {
                json_file = @"D:\文献\temp\refauto" + "\\" + N_Identif.ToString() + ".json";
            }

            JObject out_job = new JObject();
            out_job.Add("ArticleNumber", ArticleNumber);
            out_job.Add("StartPage", StartPage);
            out_job.Add("EndPage", EndPage);
            out_job.Add("CitedReferenceCount", CitedReferenceCount);
            out_job.Add("NumberofPages", NumberofPages);
            out_job.Add("PublicationYear", PublicationYear);
            out_job.Add("TimesCitedWoSCore", TimesCitedWoSCore);
            out_job.Add("180DayUsageCount", Day180UsageCount);
            out_job.Add("Since2013UsageCount", Since2013UsageCount);
            out_job.Add("Volume", Volume);
            out_job.Add("ISBN", ISBN);
            out_job.Add("TimesCitedAllDatabases", TimesCitedAllDatabases);
            out_job.Add("Abstract", Abstract);
            out_job.Add("BookSeriesSubtitle", BookSeriesSubtitle);
            out_job.Add("FundingText", FundingText);
            out_job.Add("Addresses", Addresses);
            out_job.Add("Affiliations", Affiliations);
            out_job.Add("ConferenceLocation", ConferenceLocation);
            out_job.Add("ConferenceTitle", ConferenceTitle);
            out_job.Add("ConferenceDate", ConferenceDate);
            out_job.Add("BookDOI", BookDOI);
            out_job.Add("DateofExport", DateofExport);
            out_job.Add("DocumentType", DocumentType);
            out_job.Add("EarlyAccessDate", EarlyAccessDate);
            out_job.Add("eISSN", eISSN);
            out_job.Add("IDSNumber", IDSNumber);
            out_job.Add("HighlyCitedStatus", HighlyCitedStatus);
            out_job.Add("ConferenceHost", ConferenceHost);
            out_job.Add("HotPaperStatus", HotPaperStatus);
            out_job.Add("KeywordsPlus", KeywordsPlus);
            out_job.Add("Issue", Issue);
            out_job.Add("JournalAbbreviation", JournalAbbreviation);
            out_job.Add("JournalISOAbbreviation", JournalISOAbbreviation);
            out_job.Add("Language", Language);
            out_job.Add("MeetingAbstract", MeetingAbstract);
            out_job.Add("OpenAccessDesignations", OpenAccessDesignations);
            out_job.Add("ORCIDs", ORCIDs);
            out_job.Add("PublisherAddress", PublisherAddress);
            out_job.Add("PublicationDate", PublicationDate);
            out_job.Add("PublisherCity", PublisherCity);
            out_job.Add("PubmedId", PubmedId);
            out_job.Add("PartNumber", PartNumber);
            out_job.Add("PublicationType", PublicationType);
            out_job.Add("Publisher", Publisher);
            out_job.Add("ResearcherIds", ResearcherIds);
            out_job.Add("ReprintAddresses", ReprintAddresses);
            out_job.Add("BookSeriesTitle", BookSeriesTitle);
            out_job.Add("SpecialIssue", SpecialIssue);
            out_job.Add("ISSN", ISSN);
            out_job.Add("SourceTitle", SourceTitle);
            out_job.Add("ConferenceSponsor", ConferenceSponsor);
            out_job.Add("Supplement", Supplement);
            out_job.Add("ArticleTitle", ArticleTitle);
            out_job.Add("UTUniqueWOSID", UTUniqueWOSID);
            out_job.Add("WebofScienceIndex", WebofScienceIndex);
            out_job.Add("DOI", DOI);
            out_job.Add("CitedReferences", JArray.FromObject(CitedReferences));
            out_job.Add("AuthorFullNames", JArray.FromObject(AuthorFullNames));
            out_job.Add("Authors", JArray.FromObject(Authors));
            out_job.Add("BookAuthors", JArray.FromObject(BookAuthors));
            out_job.Add("BookEditors", JArray.FromObject(BookEditors));
            out_job.Add("BookAuthorFullNames", JArray.FromObject(BookAuthorFullNames));
            out_job.Add("GroupAuthors", JArray.FromObject(GroupAuthors));
            out_job.Add("EmailAddresses", JArray.FromObject(EmailAddresses));
            out_job.Add("FundingOrgs", JArray.FromObject(FundingOrgs));
            out_job.Add("WoSCategories", JArray.FromObject(WoSCategories));
            out_job.Add("AuthorKeywords", JArray.FromObject(AuthorKeywords));
            out_job.Add("ResearchAreas", JArray.FromObject(ResearchAreas));
            out_job.Add("BookGroupAuthors", JArray.FromObject(BookGroupAuthors));

            File.WriteAllText(json_file, JsonConvert.SerializeObject(out_job));
            //// JObject obji = (JObject)JsonConvert.DeserializeObject<JObject>(File.ReadAllText(fp));  // 尖括号<>中填入对象的类名 
            return JsonConvert.SerializeObject(out_job);
        }
